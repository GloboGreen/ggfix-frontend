...
export default function OwnerEmployeeAddScreen({ navigation }) {
  ...
  const handleAdd = async (emp) => {
    if (!emp.relievedFromLastShop) {
      Alert.alert(
        'Cannot add',
        'This employee has not been relieved from their previous shop yet.'
      );
      return;
    }
    setActing(emp.id);
    try {
      ...
      Alert.alert('Added', `${emp.name} has been added to your shop.`, [
        { text: 'OK', onPress: () => navigation.goBack() },
      ]);
    } catch (e) {
      Alert.alert('Could not add', e?.body?.message || e?.message || 'Please try again.');
    } finally {
      setActing(null);
    }
  };

  const handleBlock = (emp) => {
    Alert.alert(
      'Block this employee?',
      `${emp.name} has not been relieved from their last shop. Blocking will hide them from your search results.`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              Alert.alert('Failed', e?.body?.message || e?.message || 'Please try again.');
            } finally {
              setActing(null);
            }
          },
