...
export default function AssignTechnicianScreen({ route, navigation }) {
  ...
  const onAssign = async (tech) => {
    ...
    if (!ticketId || !technicianId) {
      Alert.alert('Error', `Missing data: ticketId=${ticketId}, technicianId=${technicianId}`);
      return;
    }
    ...
    try {
      ...
      Alert.alert(
        'Technician assigned',
        'You can continue to receipt or barcode, or go back to home.',
        [
          { text: 'Continue', onPress: () => navigation.goBack() },
          { text: 'Done', onPress: () => navigation.navigate('OwnerTabs') },
        ]
      );
    } catch (e) {
