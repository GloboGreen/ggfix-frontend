...
export default function OwnerPickupSlotsScreen({ navigation }) {
  ...
  const onDelete = (slot) => {
    Alert.alert(
      'Remove slot?',
      `Remove ${dayLabel(slot.dayOfWeek)} ${normaliseTime(slot.startTime)}–${normaliseTime(slot.endTime)}?`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              Alert.alert('Failed', e?.payload?.message || e?.message || 'Please try again.');
            } finally {
              setDeletingId(null);
            }
          },
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content}>
        ...
        {editingId ? null : loading ? (
          <ActivityIndicator color="#3B4FD7" style={{ marginTop: 20 }} />
        ) : slots.length === 0 ? (
            ...
            return (
              <View key={slot.id} style={[styles.slotCard, beingEdited && styles.slotCardEditing]}>
                <View style={styles.slotIconWrap}>
                  <Ionicons name="time" size={18} color="#3B4FD7" />
                </View>
                <View style={{ flex: 1 }}>
                ...
                >
                  <Ionicons name="create-outline" size={18} color="#3B4FD7" />
                </TouchableOpacity>
                <TouchableOpacity
                  style={styles.iconBtn}
...
const styles = StyleSheet.create({
  ...
  daysSummary: { fontSize: 13, color: '#2563EB', fontWeight: '700', marginTop: 2, marginBottom: 6 },
  ...
  slotCardEditing: { borderWidth: 1, borderColor: '#3B4FD7', backgroundColor: '#EEF2FF' },
  slotIconWrap: {
    width: 32,
    height: 32,
    borderRadius: 16,
