...
export default function ChooseNearbyShopScreen({ navigation }) {
  ...
  const load = useCallback(async (isRefresh = false) => {
    ...
    try {
      const data = await shopApi.get('/shops/nearby');
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <Text style={styles.hint}>Data from GET /shops/nearby (or similar).</Text>
      <FlatList
        data={list}
        ...
        renderItem={({ item }) => (
          <TouchableOpacity style={styles.row}>
            ...
            <Text style={styles.addr}>{item.address || item.slug}</Text>
          </TouchableOpacity>
        )}
        ListEmptyComponent={<Text style={styles.empty}>No shops. Backend to provide nearby list.</Text>}
      />
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  hint: { fontSize: 12, color: '#9AA0A6', paddingHorizontal: 16, marginBottom: 8 },
  ...
  addr: { fontSize: 14, color: '#9AA0A6', marginTop: 4 },
