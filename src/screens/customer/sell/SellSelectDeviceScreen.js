...
export default function SellSelectDeviceScreen({ navigation, route }) {
  ...
  const proceed = (d) => navigation.navigate('SelectVariant', { flow: 'SELL', brandId: d.brandId, modelId: d.modelId, modelName: d.modelName, ramOptionId: d.ramOptionId, storageOptionId: d.storageOptionId, color: d.color, imei: d.imei });
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.inner}>
      ...
      {items.map((d, idx) => (
        <View key={d.id} style={styles.card}>
          ...
          <View style={styles.body}>
            <View style={{ flex: 1 }}>
              <Text style={styles.name}>{d.modelName}</Text>
      ...
      <PrimaryButton title="Select Other Device" onPress={() => navigation.navigate('SelectCategory', { flow: 'SELL' })} />
    </ScrollView>
  );
}
